      ...
      generateMeals: () => {
        ...
        if (samgakProducts.length > 0) {
          ...
          for (let dayIndex = 0; dayIndex < dates.length; dayIndex++) {
            ...
            // 음료 선택 (삼각도 월요일 요거트 필수 규칙 적용)
            const drinkGroup = pickDrinkGroup(false, prevDrinkGroup, dayOfWeek)
            prevDrinkGroup = drinkGroup
            const drinkPool = poolByGroups(drinkProducts, [drinkGroup], dayOfWeek)
            // 삼각3.5 목표원가 기준으로 음료 선택 (삼각3.5 = A+B 중 A가 음료)
            ...
            const baseCostAfterDrink = ff.cost + (drink?.cost ?? 0)
            const targetB = target3 - baseCostAfterDrink
            ...
            const desserts35 = dessertB ? [dessertB] : []
            const totalCost3 = baseCostAfterDrink + desserts35.reduce((s, x) => s + x.cost, 0)
            ...
            const totalCost4 = baseCostAfterDrink + desserts45.reduce((s, x) => s + x.cost, 0)

            // 조합 고유성 체크
            const comboIds = [drink?.id ?? '', ...desserts45.map(x => x.id)].filter(Boolean)
            if (!combinationTracker.isUnique(comboIds) && desserts45.length > 0) {
              const altPool = dessertProducts.filter(p => !usedTodayIds.has(p.id) && (p.group !== '요거트' || isYogurtAllowed(dayOfWeek)))
              if (altPool.length > 0) {
            ...
            meals3.push({ date: dateStr, compositions: { 3500: { ff, drink, desserts: desserts35, totalCost: totalCost3 } } })
            meals4.push({ date: dateStr, compositions: { 4500: { ff, drink, desserts: desserts45, totalCost: totalCost4 } } })
          }
